...
type Interface interface {
	...
	// GetValueKeys returns a list of value keys that are encrypted by the key derived from the original multihash.
	// secondHash - is a hash over the original multihash.
	GetValueKeys(secondHash multihash.Multihash) ([][]byte, bool, error)

	// GetValue returns indexer Value by the value key.
...
type Iterator interface {
	...
	Next() (multihash.Multihash, [][]byte, error)
