...
func (s *store) GetValueKeys(secondHash multihash.Multihash) ([][]byte, bool, error) {
	panic("get value keys is not supported")
}
...
func (i *iterator) Next() (multihash.Multihash, [][]byte, error) {
	...
	defer vks.Close()
	vs := make([][]byte, len(vks.keys))
	for j, vk := range vks.keys {
		vkcpy := make([]byte, len(vk.buf)-1)
		copy(vkcpy, vk.buf[1:])
		vs[j] = vkcpy
	}
	i.it.Next()
