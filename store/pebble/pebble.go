...
// defaultKeyerLength is the length of encrypted hashed + nonce generated by the default keyer, blake3Keyer.
const dhashKeyerLength = 48
...
func (s *store) GetValue(valKey []byte) (*indexer.Value, error) {
	...
	vk := keygen.valueKeyFromPayload(false, len(valKey), valKey)
...
func (s *store) PutValue(v indexer.Value, batch interface{}) ([]byte, error) {
	b := batch.(*pebble.Batch)
...
func (s *store) ValueKey(v *indexer.Value) ([]byte, error) {
	keygen := s.p.leaseBlake3Keyer()
	defer keygen.Close()
...
func (s *store) RemoveValueKey(mh multihash.Multihash, valKey []byte, batch interface{}) error {
	...
	vk := keygen.valueKeyFromPayload(true, len(valKey), valKey)

	return b.Merge(mhk.buf, vk.buf, pebble.NoSync)
}

func (s *store) RemoveValue(value *indexer.Value) error {
	valKey, err := s.ValueKey(value)
	if err != nil {
		return err
	}
	keygen := s.p.leaseBlake3Keyer()
