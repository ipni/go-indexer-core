...
func New(path string, opts *pebble.Options) (dstore.Interface, error) {
	p := newPool()
	c := &codec{
		p: p,
	}
...
func (s *store) GetValueKeys(mh multihash.Multihash) ([][]byte, bool, error) {
	...
	vkhs, err := s.vcodec.unmarshalValueKeyHashKeys(vkhbcpy)
	if err != nil {
		return nil, false, err
	}
	defer vkhs.Close()
...
func (i *iterator) Next() (multihash.Multihash, [][]byte, error) {
	switch {
	case i.it.Error() != nil:
	...
	vks, err := i.vcodec.unmarshalValueKeyHashKeys(i.it.Value())
	if err != nil {
		return nil, nil, err
	}
	defer vks.Close()
